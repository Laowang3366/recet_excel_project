...
export function ToolsConvert() {
  ...
  const historyRecords = (historyQuery.data?.records || []).slice(0, 3);

  const conversionMutation = useMutation({
    mutationKey: toolsKeys.convert(),
    mutationFn: async () => {
      if (!selectedFile) {
      ...
      return api.post<{ fileName: string; url: string; message: string }>("/api/tools/convert", formData, { silent: true });
    },
    onSuccess: (result) => {
    ...
    onError: (error: unknown) => {
      ...
      if (error instanceof ApiError && error.status === 402) {
        toast.info(error.message || `积分不足，每次转换需要 ${conversionCostPoints} 积分`);
        return;
      }
      toast.error(error instanceof Error ? error.message : "文件转换失败，请稍后重试");
    },
  });
      ...
      <section className="grid gap-5 xl:grid-cols-[1.08fr_0.92fr]">
        ...
        <div className="space-y-5">
          ...
          <LitePanel>
            ...
            />
            <div className="mt-5 space-y-3">
              {historyRecords.length > 0 ? historyRecords.map((item) => (
                <div key={item.id} className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                  <div className="truncate text-sm font-black text-slate-900">{item.sourceFileName || "-"}</div>
                  <div className="mt-2 flex flex-wrap items-center gap-2 text-xs font-semibold text-slate-500">
                    <span className="rounded-full bg-white px-2.5 py-1">{item.sourceType || "-"}</span>
                    <span>→</span>
                    <span className="rounded-full bg-white px-2.5 py-1">{item.targetType || "-"}</span>
                  ...
                  {item.resultUrl ? (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="mt-3 inline-flex h-9 items-center justify-center gap-2 rounded-xl bg-slate-900 px-3 text-xs font-black text-white transition hover:bg-slate-800"
                    >
                      <Download size={14} />
                      下载
                    </a>
